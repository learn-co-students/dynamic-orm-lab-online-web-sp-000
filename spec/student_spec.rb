...
describe Student do
  ...
  context 'has instance methods to insert data into db' do 
    ...
    describe '#save' do 
      it 'saves the student to the db' do 
        new_student.save
     #   expect(DB[:conn].execute("SELECT * FROM students WHERE name = 'Sam'")).to eq([{"id"=>1, "name"=>"Sam", "grade"=>11, 0=>1, 1=>"Sam", 2=>11}]) MY SELECT STATEMENT DOESN'T RETURN THE 0 =>  1=> 2 => AND I CAN'T FIGURE OUT WHY
  ...
  describe '.find_by_name' do 
    ...
    #  expect(Student.find_by_name("Jan")).to eq([{"id"=>1, "name"=>"Jan", "grade"=>10, 0=>1, 1=>"Jan", 2=>10}]) SAME ISSUE
  ...
  describe '.find_by' do 
    it 'executes the SQL to find a row by the attribute passed into the method' do 
     ...
     # expect(Student.find_by({name: "Susan"})).to eq([{"id"=>1, "name"=>"Susan", "grade"=>10, 0=>1, 1=>"Susan", 2=>10}]) SAME ISSUE
